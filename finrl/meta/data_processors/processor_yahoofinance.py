    ...
    def download_data(
        ...
        for tic in ticker_list:
            ...
            while (
                ...
                # if temp_df.empty:
                #     print(f"No data found for {tic} on {current_tic_start_date}")
                # else:
                #     temp_df["tic"] = tic
                #     data_df = pd.concat([data_df, temp_df])

                temp_df["tic"] = tic
                # print(data_df, temp_df)
                # data_df = pd.concat([data_df, temp_df])
                data_df = pd.concat([temp_df])
                # data_df = pd.concat(data_df, temp_df)
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.time_interval == "1d":
            ...
            for day in trading_days:
                #                NY = "America/New_York"
                # current_time = pd.Timestamp(day + " 09:30:00").tz_localize(NY).tz_convert(NY)
                current_time = pd.Timestamp(day + " 09:30:00").tz_convert(NY)
